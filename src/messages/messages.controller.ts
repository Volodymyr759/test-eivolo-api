...
export class MessagesController {
    ...
    async findById(@UserData() decodedUser: DecodedUser, @Param('id') id: string): Promise<MessageModel> {
        if (!decodedUser) {
            throw new HttpException(BAD_REQUEST, HttpStatus.BAD_REQUEST);
        }
        if (!decodedUser.user.roles.includes(Role.Admin)) {
    ...
    async create(@UserData() decodedUser: DecodedUser, @Body() createMessageDto: CreateMessageDto) {
        if (!decodedUser) {
            throw new HttpException(ACCESS_DENIED, HttpStatus.UNAUTHORIZED);
        }
    ...
    async deleteById(@UserData() decodedUser: DecodedUser, @Param('id') id: string) {
        if (!decodedUser) {
            throw new HttpException(BAD_REQUEST, HttpStatus.BAD_REQUEST);
        }
        if (!decodedUser.user.roles.includes(Role.Admin)) {
    ...
    async updateById(@UserData() decodedUser: DecodedUser, @Param('id') id: string, @Body() updateMessageDto: CreateMessageDto) {
        if (!decodedUser) {
            throw new HttpException(BAD_REQUEST, HttpStatus.BAD_REQUEST);
        }
        if (!decodedUser.user.roles.includes(Role.Admin)) {
